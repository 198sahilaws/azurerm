...
resource "azurerm_network_security_group" "cc_mgmt_nsg" {
  ...
  name                = var.cc_mgmt_nic_nsg_name != null ? element(var.cc_mgmt_nic_nsg_name, count.index) : "${var.name_prefix}-cc-mgmt-nsg-${count.index + 1}-${var.resource_tag}"
...
resource "azurerm_network_security_group" "cc_service_nsg" {
  count               = var.byo_nsg == false ? var.nsg_count : 0
  name                = var.cc_srvc_nic_nsg_name != null ? element(var.cc_srvc_nic_nsg_name, count.index) : "${var.name_prefix}-cc-service-nsg-${count.index + 1}-${var.resource_tag}"
  location            = var.location
  resource_group_name = var.resource_group
