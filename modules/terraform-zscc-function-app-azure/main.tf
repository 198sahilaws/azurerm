...
resource "azurerm_service_plan" "vmss_orchestration_app_service_plan" {
  name                = var.app_service_plan_name != "" ? var.app_service_plan_name : "${var.name_prefix}-ccvmss-${var.resource_tag}-app-service-plan"
  resource_group_name = var.resource_group
  location            = var.location
  os_type             = "Linux"
  sku_name            = "Y1"
...
resource "azurerm_log_analytics_workspace" "vmss_orchestration_log_analytics_workspace" {
  ...
  name                = var.log_analytics_workspace_name != "" ? var.log_analytics_workspace_name : "${var.name_prefix}-ccvmss-${var.resource_tag}-workspace"
  location            = var.location
  resource_group_name = var.resource_group
...
resource "azurerm_application_insights" "vmss_orchestration_app_insights" {
  name                = var.app_insight_name != "" ? var.app_insight_name : "${var.name_prefix}-ccvmss-${var.resource_tag}-app-insights"
  location            = var.location
  resource_group_name = var.resource_group
...
resource "azurerm_linux_function_app" "vmss_orchestration_app" {
  ...
  name                = var.function_app_name != "" ? var.function_app_name : "${var.name_prefix}-ccvmss-${var.resource_tag}-function-app"
  resource_group_name = var.resource_group
  location            = var.location
