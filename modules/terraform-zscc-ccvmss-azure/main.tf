...
resource "azurerm_orchestrated_virtual_machine_scale_set" "cc_vmss" {
  ...
  name                        = var.cc_vmss_name != null ? element(var.cc_vmss_name, count.index) : "${var.name_prefix}-ccvmss-${count.index + 1}-${var.resource_tag}"
  ...
  network_interface {
    name                      = var.cc_vm_srvc_nic_name != null ? element(var.cc_vm_mgmt_nic_name, count.index) : "${var.name_prefix}-ccvmss-mgmt-nic-${var.resource_tag}"
    primary                   = true
  ...
  network_interface {
    name                          = var.cc_vm_srvc_nic_name != null ? element(var.cc_vm_srvc_nic_name, count.index) : "${var.name_prefix}-ccvmss-fwd-nic-${var.resource_tag}"
